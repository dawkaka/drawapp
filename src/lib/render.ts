...
function pencilDraw(ctx: CanvasRenderingContext2D, item: Pencil) {
  ...
  for (let i = 1; i < item.points.length; i++) {
    ...
    ctx.bezierCurveTo(
      ...
      cp1.x,
      cp1.y,
      item.x + item.points[i].x,
      item.y + item.points[i].y
    );
  }
...
function rectangleDraw(ctx: CanvasRenderingContext2D, item: Rectangle) {
  ...
  const r =
    w < h ? (w / 100) * item.borderRadius : (h / 100) * item.borderRadius;
...
function diamondDraw(ctx: CanvasRenderingContext2D, item: Diamond) {
  ...
  const rVertical =
    (diagonal / 2 / 100) * item.borderRadius * Math.cos(verticalAngle);
  const rHorizontal =
    (diagonal / 2 / 100) * item.borderRadius * Math.cos(horizontalAngle);
  ctx.translate(x, y);
...
function calculateQuadraticControlPoint(
  start: Point,
  peak: Point,
  end: Point
): Point {
...
function lineDraw(ctx: CanvasRenderingContext2D, item: Line) {
  ...
  const controlPoint = calculateQuadraticControlPoint(
    startPoint,
    points[0],
    points[1]
  );
...
function arrowDraw(ctx: CanvasRenderingContext2D, item: Arrow) {
  ...
  function triangle(x: number, y: number, size: number, angle: number) {
    ...
    ctx.closePath();
    ctx.fillStyle =
      getInverseColorForTheme('#FFFFFF') === '#FFFFFF' ? '#000000' : '#FFFFFF';
    ctx.fill();
    ctx.stroke();
    ctx.restore();
  }
  ...
  if (item.structure === 'curve') {
    ...
    const arrowSize = Math.min(
      15,
      0.5 * Math.min(Math.abs(endPoint.y), Math.abs(center.x))
    );
    if (item.head !== 'none') {
...
export function renderBounds(
  ...
  switch (bounds.type) {
    ...
    case 'line':
      ...
      var controlPoint = calculateQuadraticControlPoint(
        startPoint,
        bounds.curveControl,
...
function textDraw(ctx: CanvasRenderingContext2D, item: Text) {
  ...
  for (let text of texts) {
    ...
    if (item.textStrikethrough) {
      ctx.fillRect(
        x,
        item.y + (h + item.fontSize / 4 - item.fontSize * 0.025),
        textWidth,
        item.fontSize * 0.05
      );
    }
    ...
    if (item.textUnderline) {
      ctx.fillRect(
        x,
        item.y + (h - item.fontSize / 2 + item.fontSize * 0.01),
        textWidth,
        item.fontSize * 0.05
      );
    }
...
function imageDraw(
  ...
  ctx.drawImage(
    imageData,
    item.width < 0 ? -item.x - item.width : item.x,
    item.height < 0 ? -item.y - item.height : item.y,
    item.width,
    item.height
  );
  ctx.restore();
...
export function drawMultipleSelectionBounds(
  ctx: CanvasRenderingContext2D,
